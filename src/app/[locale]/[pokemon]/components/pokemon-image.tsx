...
export const PokemonImage = ({ currentPokemon, showShiny = false, setShowShiny }: Props) => {
  const pokemonTypes = currentPokemon.types.map((t: any) => t.type.name);
  return (
    <Card className="aspect-square justify-around flex flex-col w-fit min-w-[58%] h-fit gap-1 m-0 p-2">
      <CardHeader className="text-center capitalize text-4xl font-game">
        <CardTitle>{currentPokemon.name}</CardTitle>
      </CardHeader>
      <CardDescription className="flex flex-row items-center justify-center w-full h-fit">{currentPokemon.types.map((t: any) => (
        <Image
          key={t.type.name}
          width={40}
          height={40}
      ...
      <CardContent>
        <Carousel className="w-full place-self-center rounded-md p-0" opts={{ loop: true, dragFree: false, dragThreshold: 50, containScroll: "keepSnaps" }}>
          ...
          <CarouselNext text={!showShiny ? "Shiny" : "Normal"} variant="ghost" size={"default"} func={() => setShowShiny(!showShiny)} />
        </Carousel>
      </CardContent>
    </Card>
  );
